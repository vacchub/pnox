...
/******************************************************************************/
int	l_msgget(queue_name, size, mode)
char    *queue_name;
int     size;
int     mode;
{
...
/* RET  : 0 -> 정상 송신, -1 -> ERROR                                         */
/******************************************************************************/
int	l_putmsg(qid, mtype, trans_b, trans_l, wait_mode)
int     qid, mtype;
char    *trans_b;
...
/* RET  : 0 -> 정상 송신, -1 -> ERROR                                         */
/******************************************************************************/
int	l_putmsgx(queue_name, mtype, trans_b, trans_l, wait_mode)
char    *queue_name;
int     mtype;
char    *trans_b;
int     trans_l, wait_mode;
...
/******************************************************************************/
int	l_getmsg(qid, mtype, trans_b, bsize, timeout)
int     qid, *mtype;
char    *trans_b;
int     bsize, timeout;
{
...
/* RET  : 수신한 MESSAGE DATA의 LENGTH                                        */
/******************************************************************************/
int	l_getmsgx(queue_name, mtype, trans_b, bsize, timeout)
char    *queue_name;
int     *mtype;
char    *trans_b;
int     bsize, timeout;
 ...
 * NAME : l_semopen()                                                 
 ****************************************************************************/
int l_semopen(sem_name)
char *sem_name;
{
	int semkey, semid, status;
	union semun sem_union;

	semkey = l_token(sem_name, 'S');
	if (semkey < 0)
		return(-1);
