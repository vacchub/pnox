...
int		sock;
{
	...
	if (rcvl == 0)
	{
pxsyslog("pnoxrta", "pnoxrta TCP disconnect. sock[%d]", sock);
		return(-1);
	}

	if (rcvl < PNOXHDR_L)
	{
		pxsyslog("pnoxrta", "pnoxrta less read");
		return(-2);
	}
	...
	{
		pxsyslog("pnoxrta", "pnoxrta format error");
		return(0);
	}
	...
	memset(tHandle->call, 0x00, strlen(tHandle->call));
	...
	if (retc < 0)
	{
		pxsyslog("pnoxrta", "pnoxrta pxsend error %d", retc);
	}	
...
int		sock;
{
	...
	if (retc < 0)
	{
		pxsyslog("pnoxrta", "service_tr pxrecv %d", retc);
		return(-1);
	}
...
pxsyslog("pnoxrta", "service_tr devf[%d] pxid[%d] name[%.8s] trnm[%.8s] rcvl[%d] rcvb[%.*s]",
tHandle->devf, tHandle->pxid, tHandle->name, mHandle.trnm, mHandle.rcvl, 
mHandle.rcvl, mHandle.rcvb);
	...
	if (retc != bufl)
	{
		pxsyslog("pnoxrta", "service_tr write %d(%d)", retc, errno);
		return(-2);
	}
...
int		sock;
{
    ...
    if (retc != bufl)
    {
        pxsyslog("pnoxrta", "service_poll write %d(%d)", retc, errno);
        return(-1);
    }
