...
/*----------------------------------------------------------------------------*/
/* pxtoken()       IPC를 위한 시스템에서 유일한 TOKEN VALUE를 산출            */
/* pxmsgget()      입력된 QUEUE NAME의 MESSAGE QUEUE를 얻는다.                */
/* pxshmat()       Attach exist shared memory.                                */
/* pxputmsg()      TRANSACTION MESSAGE를 입력된 QUEUE ID 로 전달              */
/* pxputmsgx()     TRANSACTION MESSAGE를 입력된 QUEUE NAME의 QUEUE로 전달     */
/* pxgetmsg()      TRANSACTION MESSAGE를 입력된 QUEUE ID에서 수신             */
/* pxgetmsgx()     TRANSACTION MESSAGE를 입력된 QUEUE NAME의 QUEUE로          */
/*                 부터 수신 처리                                             */
/******************************************************************************/
...
/******************************************************************************/
int	pxmsgget(queue_name, size, mode)
char    *queue_name;
int     size;
int     mode;
{
        ...
        if ((w_qkey = pxtoken(queue_name, 'Q')) == -1)
                return (-1);
...
char    *shm_name;
{
        ...
        if ((w_mkey = pxtoken(shm_name, 'M')) == -1)
                return (NULL);
...
/* RET  : 0 -> 정상 송신, -1 -> ERROR                                         */
/******************************************************************************/
int	pxputmsg(qid, mtype, trans_b, trans_l, wait_mode)
int     qid, mtype;
char    *trans_b;
...
/* RET  : 0 -> 정상 송신, -1 -> ERROR                                         */
/******************************************************************************/
int	pxputmsgx(queue_name, mtype, trans_b, trans_l, wait_mode)
char    *queue_name;
int     mtype;
char    *trans_b;
int     trans_l, wait_mode;
{
        ...
        w_qid = pxname2id(queue_name);
        if (w_qid == -1)
                return (-1);

        return (pxputmsg(w_qid, mtype, trans_b, trans_l, wait_mode));
#endif
	return(0);
}
/******************************************************************************/
...
/* IN   : qid           수신할 MESSAGE queue id로 에 의하여 pxname2id         */
/*                      RETURN된 QUEUE id 값이다.                             */
...
/******************************************************************************/
int	pxgetmsg(qid, mtype, trans_b, bsize, timeout)
int     qid, *mtype;
char    *trans_b;
int     bsize, timeout;
{
...
/* RET  : 수신한 MESSAGE DATA의 LENGTH                                        */
/******************************************************************************/
int	pxgetmsgx(queue_name, mtype, trans_b, bsize, timeout)
char    *queue_name;
int     *mtype;
char    *trans_b;
int     bsize, timeout;
{
        ...
        if (w_qid == -1)
                return (-1);
        return (pxgetmsg(w_qid, mtype, trans_b, bsize, timeout));
}
...
/* NOTE: 공통 프로그램에서만 사용                                             */
/******************************************************************************/
static int pxname2id(queue_name)
char    *queue_name;
{
        int      w_qid;
        register int ii;
        ...
        w_qid = pxmsgget(queue_name, 0, 0666);
        if (w_qid == -1)
                return (-1);
        if (s_queue_n < 32)
        {
 ...
 * NAME : pxsemopen()                                                 
 ****************************************************************************/
int pxsemopen(sem_name)
char *sem_name;
{
	int semkey, semid, status;
	union semun sem_union;

	semkey = pxtoken(sem_name, 'S');
	if (semkey < 0)
		return(-1);
